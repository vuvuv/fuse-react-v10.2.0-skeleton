...
const schema = z
	.object({
		...
		nik: z.string().nonempty('Please enter your nik.').min(9, 'Password is too short - should be 8 chars minimum.'),
		password: z
			.string()
...
function JwtSignUpTab() {
	...
	function onSubmit(formData: SignUpPayload) {
		const { displayName, nik, password } = formData;
		jwtService
			.signUp({
				displayName,
				password,
			...
			.catch((_errors: { type: 'nik' | 'password' | `root.${string}` | 'root'; message: string }[]) => {
